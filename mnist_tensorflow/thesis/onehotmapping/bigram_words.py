...
output_file = open("bigram_encounters.txt", 'w')
...
matrix = np.array(np.zeros((len(splitted_bigrams.values()), len(encounters)))).reshape(596, 371)
...
df.to_csv('bigram_df.csv', index=True, header=True, sep=' ')

output_file = open("bigram_matrix.txt", 'w')
